...
def gen_xiaoqu_insert_command(info_dict):
    ...
    :return:
    """
    info_list=[u'小区名称',u'大区域',u'小区域',u'小区户型及时间']
    t=[]
    for il in info_list:
        if il in info_dict:
...
def xiaoqu_spider(db_xq, url_page=u"http://bj.lianjia.com/xiaoqu/changping/"):
    ...
    for xq in xiaoqu_list:
        ...
        if info:
            ...
            info_dict.update({u'小区户型及时间': info[2]})
...
def xiaoqu_chengjiao_spider(db_cj,xq_name=u'名佳花园三区'):
...
if __name__=='__main__':
    command = "create table if not exists xiaoqu (name TEXT primary key UNIQUE, regionb TEXT, regions TEXT,style TEXT)"
